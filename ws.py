...
EMA_WINDOW = 2.0  # seconds time-constant for EMA
...
def update_ema_and_state(key: str, raw_value: float):
    ...
    if prev is None:
        ...
        dt = now - prev
        α  = 1 - math.exp(-dt/EMA_WINDOW)
        cell["ema"] += α*(raw_value - cell["ema"])
    cell["last_ts"] = now

# ── 2) Shared queue & WS state ──────────────────────────────────────────────
message_queue = asyncio.Queue()
app = FastAPI()
clients: list[WebSocket] = []

def broadcast(key: str):
    """Send CELLS[key]['ema'] formatted to all WS clients."""
    cell = CELLS[key]
...
# ── 5) UDP listener → enqueue messages ──────────────────────────────────────
...
# ── 6) Processor: parse → EMA → broadcast ─────────────────────────────────
async def processor():
    while True:
